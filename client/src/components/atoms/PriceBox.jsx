import { calcPorcentage, numberWithCommas } from "../../utils/formatters";

const PriceBox = ({ price, original_price, className, testId, full }) => {
  return (
    <div className={className}>
      {original_price !== 0 && original_price > price ? (
        <div className="prices-box">
          ...
          <p className="line-through smaller p-0 m-0">
            $ {numberWithCommas(original_price)}
          </p>
          <div className="d-flex flex-fill justify-content-start align-items-center">
            <p data-testid={testId} className="price">
              $ {numberWithCommas(price)}
            </p>
            <p data-testid={testId} className="discount-price">
              {calcPorcentage(original_price, price)}% OFF
            </p>
          </div>
        </div>
      ) : (
